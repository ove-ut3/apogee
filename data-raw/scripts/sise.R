...
conv_etape_sise <- readxl::read_excel("data-raw/data/Etape.xlsx", "Etape_sise", skip = 1) %>% 
...
sise_diplome <- readxl::read_excel("data-raw/data/Diplome.xlsx", "Diplome_sise", skip = 1) %>% 
  patchr::rename(impexp::access_import("_rename", access_base_path)) %>% 
  patchr::transcode(impexp::access_import("_contents", access_base_path)) %>% 
  dplyr::filter(annee >= 2007) %>% 
  dplyr::select(-annee) %>% 
...
sise_discipline <- impexp::access_import("sise_discipline", "data-raw/data/Tables_ref.accdb")

usethis::use_data(sise_discipline, overwrite = TRUE)

#### SISE - libellé diplôme ####

sise_diplome_lib <- readxl::read_excel("data-raw/data/Diplome.xlsx", "Diplome_sise_lib", skip = 1) %>% 
  patchr::rename(impexp::access_import("_rename", access_base_path)) %>% 
  patchr::transcode(impexp::access_import("_contents", access_base_path))
...
sise_diplome_type <- impexp::access_import("sise_diplome_type", "data-raw/data/Tables_ref.accdb")
